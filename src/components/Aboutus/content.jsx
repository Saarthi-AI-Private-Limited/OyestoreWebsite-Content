  ...
  const whyOyestoreFeatures = [
    {
      ...
      desc: "No hidden costs, no shady plans. Every detail is laid out upfront.",
    },
    {
      icon: <Users className="w-8 h-8" />,
      title: "Founder-Led",
      desc: "Every trip is led by the Oyestore founder – because we believe in being with our community.",
    },
    {
      icon: <Camera className="w-8 h-8" />,
      title: "Influencer Onboarded",
      desc: "Each trip features popular travel content creators for networking and fresh content.",
    },
    {
      icon: <PalmTree className="w-8 h-8" />,
      title: "Weekend-Focused",
      desc: "Quick getaways that respect your 9-to-5.",
    },
    {
      icon: <Bus className="w-8 h-8" />,
      title: "Multi-mode Options",
      desc: "Self-drive, bus, or quick flights – we adapt to you.",
    },
  ];
  ...
  const tripTypes = [
    {
      ...
      title: "Road Trips",
      desc: "(Self-drive & Shared)",
    },
    {
      icon: <Bus className="w-8 h-8" />,
      title: "Bus Trips",
      desc: "(Sleeper & Semi-sleeper)",
    },
    {
      icon: <Plane className="w-8 h-8" />,
      title: "Flight Getaways",
      desc: "(Domestic & Short International)",
    },
    {
      icon: <Waves className="w-8 h-8" />,
      title: "Experience Themes",
      desc: "Beachside Yoga, Hidden Treks, Wellness Retreats, Backpacking Blitzes",
    },
  ];
      ...
      <div className="max-w-6xl mx-auto space-y-20 relative z-10">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-slate-300 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
            Oyestore is a Bangalore-based travel collective redefining weekend
            escapes with curated, founder-led trips that blend local culture,
            community vibes, and influencer energy. We’re not just travel
            planners — we’re memory makers.
          </p>
        </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {whyOyestoreFeatures.map(({ icon, title, desc }, idx) => (
              <Card
                key={idx}
                className="bg-white/10 border border-white/20 text-white p-6 text-center hover:scale-[1.02] transition-all duration-300 rounded-xl shadow-md shadow-white/5"
              >
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {tripTypes.map(({ icon, title, desc }, idx) => (
              <Card
                key={idx}
                className="bg-white/10 border border-white/20 text-white p-6 text-center hover:scale-[1.02] transition-transform duration-300 rounded-xl"
              >
                <div className="text-pink-400 mb-4 flex justify-center">
                  {icon}
                </div>
        ...
        {/* <footer className="text-center text-sm text-slate-400 mt-12 space-y-2">
          ...
          <p className="flex items-center justify-center gap-2">
            <Instagram className="w-4 h-4" /> Instagram: @oyestore.in (coming
            soon)
          </p>
          <p className="flex items-center justify-center gap-2">
